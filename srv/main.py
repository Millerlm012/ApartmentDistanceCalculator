...
def gather_inputs():
    city_state = input(
        "Please enter the city and state you're wanting to find apartments in (eg. Des Moines, IA): "
    )
    s = city_state.split(", ")
    city = s[0]
    state = s[1]
    ...
    destination_address = input(
        "Please enter the gym address you're wanting to calculate the apartment distance from: "
    )

    return city, state, destination_address


def main(city, state, destination_address, cookie):
    apartments = []
    page_count = 1
    apartment_count = 0

    print(f"Fetching apartments for {city}, {state}...")
    html = query_apartments(city, state, cookie)
    apartments, total_pages = parse_apartments(html, apartments)
    print(
        f"Page 1 of {total_pages} fetched! ({len(apartments) - apartment_count} added)"
    )
    apartment_count = len(apartments)
    page_count += 1

    while page_count <= total_pages:
        apartments, _ = parse_apartments(
            query_apartments(city, state, cookie, page_count), apartments
        )
        print(
            f"Page {page_count} of {total_pages} fetched! ({len(apartments) - apartment_count} added)"
        )
        apartment_count = len(apartments)
        page_count += 1
    ...
    print(
        f"Calculating each apartments distance from specified destination address ({destination_address})..."
    )
    batches = math.floor(
        len(apartments) / 25
    )  # NOTE: 25 is used because that's the max amount of origin's we can do per request
    ...
    for i in range(batches + 1):
        ...
        apartments_batch = distance_matrix(
            apartments_batch, destination_address, DISTANCE_MATRIX_KEY
        )
    ...
    apartments_sorted = sorted(
        apartments, key=lambda obj: float(obj["distance"].split(" ")[0])
    )
...
if __name__ == "__main__":
    ...
    main(city, state, destination_address, COOKIE)
